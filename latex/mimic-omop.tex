...
%Abstract
\abstract{%
In the era of big data, Intensive Care Unit (ICU) is highly susceptible to gain
from computer realtime modeling based on patient close monitoring. MIMIC is yet
the first free-access ICU database. Numerous studies have shown that Common
Data Models (CDM) improve database research by allowing code, tools, experience
sharing. OMOP CDM is spreading worldwide. In this paper we evaluate the process
to transform MIMIC into OMOP, the quality of the transformation, the benefits
for analysis and the gains an potentials of the contributions. Finally the
resulting MIMIC-OMOP dataset is ready for reproductible research and is the
first open-access OMOP dataset with real de-identified patient data available.
}
