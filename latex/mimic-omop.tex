...
allows multicentric research and catalyse research in general by allowing
sharing practices, code, and tools \cite{todo}.  On the other hand some studies
have shown that results are not replicable from one to another database
\cite{omop-replicability} and that keeping the local conceptual model
\cite{fhir-deep} and structure \cite{imi-protect} of database for research
leads to better outcomes, a dozen of CDM have emerged.
...
specify which terminology to implement. In terms of structural CDM 
OMOP relational model can be materialized into csv format and stored in any relational database when FHIRlast but
not least its graph nature complicates the end user preprocessing skills.

Still some questions remains that we propose to answer such the
