...
OMOP CDM v5.3.1 which defines 15 standardized \textit{clinical} data tables, 3
\textit{health} system data tables, 2 \textit{health economics} data tables, 5 tables
for \textit{derived} elements and 12 tables for standardized \textit{vocabulary}. 
We didn’t use the health economics data tables (not provided by MIMIC)

All the process is available freely on the github website \cite{mimic-omop-website}.
...
PGSQL (PGSQL) instance and a subset of 100 patient chosen upon their
...
such \textit{admissions}, \textit{transfers} and \textit{icustays}. OMOP centralizes this
information in the \textit{visit\_detail}. As a result some hard transformation
...
\textit{visit\_detail} pour each stay in ICU (based of the transfers table) whereas
...
information were not provide (\textit{start/end\_datetime} for \textit{drug\_exposure}).
